...
//  IOSSchoolProgect
//
//  Created by maxvologin on 29.04.2022.
//
...
protocol ImageNetworkManager {
    func getImage(urlString: String, competeion: ((Data?, Error?) -> ())?)
}

extension NetworkManager: ImageNetworkManager {
    func getImage(urlString: String, competeion: ((Data?, Error?) -> ())?) {
        dataRequest(url: urlString, method: .get, onRequestCompleted: competeion)
    }
}
